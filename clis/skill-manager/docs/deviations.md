...
| D-009 | Dry runs do not mutate target deployments or command-requested configuration/backup state. Discovery cache refresh plus startup layout and schema migration are maintenance and may persist before a dry-run plan is emitted. | Strengthens remove/update dry-run tests while preserving safe adoption of the consolidated layout. |
...
| D-014 | Support global and exact-CWD project target scopes, with project deployments overriding global deployments and explicit scope required where noninteractive inference is unsafe. | New capability beyond the global-only Python manager. |
| D-015 | Persist root-relative target templates in schema v2 and automatically migrate the former absolute-path configuration and flat storage layout into `~/.skill-manager/`. | New portability and storage-safety behavior beyond the Python manager. |
| D-016 | Provide exact-byte configuration backups plus explicit `configs reset` and `configs restore` confirmations. | New recoverability behavior beyond the Python manager. |
| D-017 | Accept Python-style positional skill patterns for load, update, remove, and resolve, using each command's documented candidate universe. | New batch-operation capability beyond the Python CLI's positional exact-name handling. |
